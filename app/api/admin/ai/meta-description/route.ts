/**
 * Meta Description Generation API Endpoint
 * POST /api/admin/ai/meta-description
 */

import { NextRequest, NextResponse } from "next/server";
import { getServerSession } from "@/lib/auth";
import { generateMetaDescription, safeAiCall, isGeminiConfigured, recordAiUsage } from "@/lib/ai";
import { z } from "zod";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rateLimitResult = await checkRateLimit(request, {
      limit: 20,
      window: 3600,
      identifier: `ai:meta-description:${session.user.id}`,
    });
    ...
    const result = await safeAiCall(() =>
      generateMetaDescription(validation.data)
    );

    if (!result.success) {
      return NextResponse.json(
        { error: result.error?.message || "فشل في توليد وصف الميتا" },
        { status: 500 }
      );
    }
