...
- راعِ مبادئ E-E-A-T: الخبرة (Experience) والتخصص (Expertise) والسلطة (Authority) والمصداقية (Trust)
- استشهد بمصادر موثوقة وأرقام دقيقة لتعزيز المصداقية
- ابنِ المحتوى على الهيكل الصحفي (الهرم المقلوب: الأهم أولاً)
- اكتب بأسلوب يسهل على محركات البحث الذكية (GEO) استخراج إجابات مباشرة`;
...
تحقق من النقاط التالية أيضاً:
- E-E-A-T: هل يوجد اقتباس مباشر من خبير أو مصدر رسمي؟
- E-E-A-T: هل يوجد رابط لمصدر موثوق (حكومي، تعليمي، وكالة أنباء)؟
- الهيكل الصحفي: هل تجيب الفقرة الأولى على من/ماذا/متى/أين؟
- الهيكل: هل توجد خاتمة أو قسم خلاصة؟
- GEO: هل يوجد قسم أسئلة شائعة؟
- GEO: هل يوجد قسم "أبرز النقاط" أو ملخص؟

أعد JSON فقط بالشكل التالي:
{
  "suggestions": [
    {
      "type": "title|meta|content|keyword|structure|eeat|geo",
...
// ============================================
// Article Rewrite Prompts (Unified Workflow)
// ============================================

const ARTICLE_TYPE_INSTRUCTIONS: Record<string, string> = {
  news: 'خبر عاجل - اكتب بأسلوب إخباري سريع ومباشر، ركز على الحقائق الأساسية والتفاصيل العاجلة',
  report: 'تقرير - اكتب بأسلوب تحليلي معمق، قدم خلفية شاملة وتحليل للسياق',
  investigation: 'تحقيق - اكتب بأسلوب تحقيقي استقصائي، قدم أدلة وشهادات وتحليل معمق',
  opinion: 'رأي - اكتب بأسلوب رأي وتحليل شخصي، قدم وجهة نظر واضحة مع حجج داعمة',
  article: 'مقال - اكتب بأسلوب صحفي احترافي متوازن، اجمع بين المعلومات والتحليل',
};

export function buildRewriteArticlePrompt(data: {
  title: string;
  content: string;
  focusKeyword: string;
  seoScore: number;
...
العنوان الأصلي: ${data.title}
الكلمة المفتاحية الرئيسية: ${data.focusKeyword}
نوع المقال: ${articleTypeInstruction}
درجة SEO الحالية: ${data.seoScore}/100
رقم التحسين: ${data.iteration}
...
المطلوب:
1. أعد كتابة المقال بالكامل بصيغة HTML صالحة (استخدم <h2>, <h3>, <p> للفقرات)
2. اتبع هيكل الهرم المقلوب: المعلومات الأهم في البداية
3. اكتب مقدمة قوية وجذابة (80-120 كلمة) تحتوي على الكلمة المفتاحية
4. وزّع الكلمة المفتاحية "${data.focusKeyword}" بكثافة 1-2% بشكل طبيعي
5. استخدم عناوين فرعية <h2> و <h3> واضحة وجذابة
6. اجعل الفقرات قصيرة جداً (2-3 جمل، متوسط أقل من 40 كلمة للفقرة الواحدة)
7. أضف جمل انتقالية بين الفقرات
8. اختم بخاتمة قوية تلخص النقاط الرئيسية
9. عالج مشاكل SEO المذكورة أعلاه
10. إذا كان العنوان الأصلي يتجاوز 65 حرفاً أو أقل من 50 حرفاً، يجب كتابة عنوان جديد بين 50-65 حرفاً بالضبط يحتوي الكلمة المفتاحية
11. حافظ على جميع المعلومات والحقائق الأساسية
12. استخدم أسلوباً صحفياً احترافياً بالعربية الفصحى

تحسينات GEO (لمحركات البحث الذكية مثل ChatGPT وPerplexity):
13. اكتب الفقرة الأولى بأسلوب "الإجابة أولاً": ابدأ بحقيقة مباشرة تجيب على من/ماذا/متى/أين (تجنب البدء بـ "هناك" أو "تشهد" أو "يعتبر" أو "تعد" أو أي عبارات مبهمة)
14. استخدم جملاً قصيرة (متوسط 15-20 كلمة) لتسهيل الاستخراج من محركات الذكاء الاصطناعي
15. يجب تنظيم المعلومات القابلة للتعداد (أسباب، نتائج، خطوات، مميزات) في قوائم باستخدام <ul> أو <ol> مع <li>
16. يجب إضافة قسم <h2>أسئلة شائعة</h2> في نهاية المقال مع إجابات مختصرة على الأسئلة المتوقعة
17. يجب إضافة اقتباس واحد على الأقل من مصدر أو خبير باستخدام علامات «نص الاقتباس هنا» أو وسم <blockquote>نص الاقتباس</blockquote>
18. يجب تضمين أرقام وإحصائيات ونسب مئوية بمعدل لا يقل عن رقمين لكل 100 كلمة (أعداد، تواريخ، نسب، إحصائيات، أرقام دقيقة)

مشاكل GEO المحددة التي يجب معالجتها في هذا المقال:
${data.geoTopIssues.length > 0 ? data.geoTopIssues.map((issue, i) => `${i + 1}. ${issue}`).join('\n') : 'لا توجد مشاكل GEO محددة'}

قواعد التنسيق:
- استخدم <h2> للعناوين الرئيسية و <h3> للعناوين الفرعية
- استخدم <p> لكل فقرة
- لا تستخدم <h1> (العنوان الرئيسي منفصل)
- لا تضف class أو style للعناصر
- اجعل المحتوى جاهزاً للنشر مباشرة

أعد JSON فقط بالشكل التالي:
{
  "rewrittenContent": "<h2>العنوان الأول</h2><p>الفقرة الأولى...</p>...",
  "rewrittenTitle": "عنوان محسّن بين 50-65 حرفاً يحتوي الكلمة المفتاحية — أو null إذا كان العنوان الأصلي ضمن 50-65 حرف",
  "changesSummary": [
    "تحسين 1: وصف التحسين",
    "تحسين 2: وصف التحسين",
    "تحسين 3: وصف التحسين"
...
مهم جداً:
- أعد JSON صالح ومكتمل فقط، بدون أي نص قبله أو بعده
- المحتوى المعاد كتابته يجب أن يكون HTML كامل وجاهز للنشر
- لا تختصر المحتوى - حافظ على طول مشابه أو أطول
- إذا كان رقم التحسين > 2، حافظ على أي تعديلات يدوية قد أجراها المستخدم`;
}

/**
 * FAQ Generation Prompt
 * Generates 4-6 question/answer pairs from article content for GEO + featured snippets
 */
export function buildFaqGenerationPrompt(data: {
  title: string;
  content: string;
  focusKeyword?: string;
}): string {
  return `اقرأ المقال التالي وأنشئ 4-6 أسئلة شائعة (FAQ) مع إجاباتها المختصرة:
...
المتطلبات:
- اكتب أسئلة حقيقية قد يسألها القارئ
- الإجابات: 30-80 كلمة لكل إجابة — مباشرة ومفيدة
- ابدأ الأسئلة بـ: كيف، لماذا، ما هو، ما هي، متى، أين، هل، ماذا
- تجنب الأسئلة العامة — اجعلها خاصة بموضوع المقال
- استخدم الأسئلة التي ستظهر في محركات AI مثل ChatGPT وPerplexity
- كل سؤال يجب أن يُجيب على استفسار محتمل من القارئ
  ...
  "faqs": [
    {
      ...
      "answer": "الإجابة المختصرة والمباشرة في 30-80 كلمة."
    }
  ]
}
 ...
 * Generates 3-5 bullet points summary for GEO + AI engine extraction
 */
export function buildKeyTakeawaysPrompt(data: {
  title: string;
  content: string;
}): string {
  return `اقرأ المقال التالي واستخرج أبرز 3-5 نقاط يجب أن يعرفها القارئ:
...
المتطلبات:
- كل نقطة: جملة واحدة واضحة ومباشرة (10-20 كلمة)
- ركز على المعلومات الأكثر قيمة وفائدة للقارئ
- استخدم أرقاماً وحقائق محددة إن أمكن
- تجنب العبارات العامة — كن دقيقاً وملموساً
- رتب النقاط من الأهم إلى الأقل أهمية
- اكتب HTML جاهز للإدراج مباشرة في المحرر

أعد JSON فقط بالشكل التالي:
{
  "takeaways": ["النقطة الأولى", "النقطة الثانية", "النقطة الثالثة"],
  "html": "<h2>أبرز النقاط</h2><ul><li>النقطة الأولى</li><li>النقطة الثانية</li><li>النقطة الثالثة</li></ul>"
}
