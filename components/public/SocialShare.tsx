...
export function SocialShare({ title, url, variant = 'inline' }: SocialShareProps) {
      ...
      >
        ...
        <div className="p-3 flex flex-col gap-2">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 px-3 py-2 text-sm font-medium transition-all duration-200 rounded-none"
            style={{
              background: copied ? 'var(--accent-light)' : 'transparent',
              color: copied ? 'var(--accent)' : 'var(--muted-foreground)',
            }}
            onMouseEnter={e => {
              if (!copied) (e.currentTarget as HTMLElement).style.background = 'var(--muted)';
            }}
            onMouseLeave={e => {
  ...
  /* ── Inline variant (mobile footer) — horizontal row ── */
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-semibold text-foreground">مشاركة المقال</h3>
      <div className="flex flex-wrap gap-2">
        <a
          href={shareLinks.whatsapp}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 px-3 py-2 border border-border-subtle hover:border-accent hover:text-accent transition-colors text-sm"
...
function SidebarShareLink({
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="flex items-center gap-3 px-3 py-2 text-sm font-medium text-muted-foreground transition-all duration-200 group"
